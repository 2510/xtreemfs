...
if [ ! -e $XTREEMFS_LOG_DIR ]; then
    mkdir -p $XTREEMFS_LOG_DIR
    chown -R $XTREEMFS_USER $XTREEMFS_LOG_DIR
fi

if [ -e $XTREEMFS_ETC ]; then
    group=`stat -f %Sg $XTREEMFS_ETC 2>/dev/null`
    ...
    for file in `ls $XTREEMFS_ETC/*.properties 2>/dev/null`; do
      if [ -f $file -a "$(stat -f %p $file)" != "640" ]; then
    ...
    then
        if [ "$(stat -c %p "$XTREEMFS_ETC/truststore/")" != "750" ]
        then
